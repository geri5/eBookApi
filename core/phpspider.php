...
class phpspider
{
    ...
    function __construct($configs = array())
    {
        ...
        // 是否保存log info
        log::$log_write = isset($configs['log_write']) ? $configs['log_write'] : true;

        $configs['name'] = isset($configs['name']) ? $configs['name'] : 'ESpider';
        ...
        $configs['site_name'] = isset($configs['site_name']) ? $configs['site_name'] : $configs['name'];
        // csv、sql、db
    ...
    public function add_scan_url($url, $options = array(), $allowed_repeat = true, $type = '', $attach = array())
    {
        // 投递状态
        $status = false;
        ...
        if ($type == 'list' || $this->is_list_page($url)) {
            $link['url_type'] = 'list_page';
            $status = $this->queue_lpush($link, $allowed_repeat);
        } elseif ($type == 'content' || $this->is_content_page($url)) {
     ...
     */
    public function add_url($url, $options = array(), $depth = 0, $type = '', $attach = array())
    {
        // 投递状态
        $status = false;
        ...
        if ($type == 'content' || $this->is_content_page($url)) {
            $link['url_type'] = 'content_page';
            $status = $this->queue_lpush($link);
        }
        if ($status) {
    ...
    public function parse_command()
    {
        ...
        switch ($command) {
            ...
            // 未知命令
            default :
                exit("Unknown Command,Please Usage: php yourfile.php {start|start -d|stop|status|kill}\n");
        }
    }
    ...
    public function start()
    {
        ...
        // {
        // log::error('PHP 5.4+ is required, currently installed version is: ' . phpversion());
        // exit;
        // }
        ...
        if (log::$log_show) {
            ...
            $header .= "\n[ " . self::$configs['name'] . " Spider ] is started... Site:" . self::$configs['site_name'] . "\n\n";
            $header .= "  * ESpider: A novel spider which is based on PHPSpider\n";
        ...
        foreach (self::$configs['scan_urls'] as $url) {
            // 允许重复
            $this->add_scan_url($url, array(), true);
        }
    ...
    public function link_uncompress($link)
    {
        $link = array(
            ...
            'attach' => isset($link['attach']) ? $link['attach'] : array(),
        );
    ...
    {
        $fields = isset($configs['fields']) ? $this->get_fields(self::$configs['fields'], $html, $url, $page) : null;
    ...
    public function check_cache()
    {
        ...
        if ($count != 0) {
            ...
            $arg = self::$continue; // 暂时不能继续执行，没存储链接的规则，可能会导致出错。这里先使用self::$continue，默认为n，留给以后在入口文件指定是否需要继续
            if ($arg == 'n') {
                foreach ($keys as $key) {
    ...
    public function queue_lpush($link = array(), $allowed_repeat = false)
    {
        ...
        if (self::$use_redis) {
            ...
            $key = md5($url);
            if (!array_key_exists($key, self::$collect_urls) || $allowed_repeat) {
    ...
    public function display_ui()
    {
        ...
        $display_str = "\033[1A\n\033[K-----------------------------\033[47;30m ESPIDER COLLECTED {$this->tnum} times \033[0m-----------------------------\n\033[0m";
        //$display_str = "-----------------------------\033[47;30m PHPSPIDER \033[0m-----------------------------\n\033[0m";
        $run_time_str = util::time2second(time() - self::$time_start, false);
        $display_str .= 'ESpider version:' . self::VERSION . "          site name:" . self::$configs['site_name'] . "\n";
     ...
     * setScanUrls
     *
     * @param array $scanUrls
     */
    public function setScanUrls($scanUrls = array())
    {
        self::$configs['scan_urls'] = $scanUrls;
    }

...
class util
{
    ...
    public static function get_hash($n, $s = 50)
    {
        $hash = sprintf("%u", crc32($n));
        $hash = intval(fmod($hash, $s));
        return $hash;
    }
     ...
     * @return mixed
     */
    public static function space_html2text($text)
    {
        // &emsp;替换成四个空格
    ...
    public static function parse_jsonp($jsonp, $array = true)
    {
        $start = strpos($jsonp, '(') + 1;
        $length = strrpos($jsonp, ')') - $start;
        $jsonp = substr($jsonp, $start, $length);
        $return = json_decode($jsonp, $array) ?: false;
        if ($return) {
            return $return;
        }
        $jsonp = preg_replace('@([\w_0-9]+):@sU', '"$1":', $jsonp); // 这里有BUG，双引号中的内容也会被替换，比如http:
        return json_decode($jsonp, $array);
    }
